...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:nabuto\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=556592559365:+55 659255-9365\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '/','*','.'
blocked = []
limitawal = '999999999'
...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `adeus volte quando quiser o grupo vai estar à sua espera @${num.split('@')[0]} 😣✋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
					wait: '❬❗❭ CASO NÃO OBTENHA TENTE NOVAMENTE',
					success: '️❬ ✔ ❭ TA AI 🖤',
					...
					levelnol: '*error* 0 °-°',
					error: {
				stick: '*Bem, falhe, tente novamente lindo (a)^_^*',
				Iv: '𝗠𝗮𝗮𝗳 𝗹𝗶𝗻𝗸 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱☹️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos lindo (a)! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUMS LINDO (A)*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD BOT VINI*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[❗] Proprietário? Este é um recurso especial para o proprietário do bot Vinícius ❌',
					ownerB: '[❗] Proprietário? Este é um recurso especial para o proprietário do bot Vinícius❌',
					...
					admin: '[❗] Este comando só pode ser usado por administradores de grupo menbro comun! ❌',
					Badmin: ' [❗] Este comando só pode ser usado quando o bot se torna administrador lindo (a)! ❌',
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["556592559365@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"556592559365@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["556592559365@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["556592559365@s.whatsapp.net"]//ubah nomor lo
			const premium = ["556592559365@s.whatsapp.net","559784130980@s.whatsapp.net","557599941062@s.whatsapp.net","558598669655@s.whatsapp.net","5511949051934@s.whatsapp.net","554792091566@s.whatsapp.net","558699541889@s.whatsapp.net","559294313229@s.whatsapp.net","558396442769@s.whatsapp.net"]
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("bosta")){
		...
		if (messagesC.includes("bot")){
			...
			client.updatePresence(from, Presence.composing)
			data = fs.readFileSync('./src/botvinijokes.js');
...
//@bot-viniYT					
    ...
    break
            ...
            case 'rapL':
				...
				case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (args.length < 1) return reply(`Parameter Salah\nCommand : ${prefix}daftar nama|umur\nContoh : ${prefix}daftar BOTVINI|14`)
...
break
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOO BOT VINI 👑* 」\n\`\`\``
					no = 0
					...
					break
				case 'desligar':
				if (!isOwner) return reply('Comando so para o Vinícius?')
...
// recursos anime, random @bot-vini domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu criador é o Vinícius chama ele vai"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT BOT-VINI✨* 」\n*│+ wa.me/556592559365*╰──────*「 *BOT-VINI* 」*────*\n\n*_SE QUER SER ADMIN DO BOT BOT-VINI_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					client.sendMessage(from, sa, image, { quoted: mek, caption: '*╭────*「 *PREMIUM USER👑* 」\n*│+ wa.me/5565992559365*╰──────*「 * BOT-VINI* 」*────*\n\n*_SE QUER SER UM USUÁRIO PREMIUM DO BOT-VINI_*\n*_Ketik #daftarvip*' })
					break
					case 'cekmod': 
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do bot vini:*\n\n 'https://youtu.be/lmQNnHs-Jws})
					break
				case 'nsfwloli1':
					memein = await kagApi.memeindo()
				...
				case 'boanoite':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'bom dia lindo (a) ❤️'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* BOT-VINI\n*Y wa.me/+556592559365\n*\n\nEspero que tenham gostado do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
